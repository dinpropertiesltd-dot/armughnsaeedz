    ...
    <div className="space-y-6 sm:space-y-10 animate-in fade-in duration-700 pb-10">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-slate-50/50 text-slate-400 text-[9px] uppercase font-black tracking-[0.2em] border-b border-slate-100">
                    ...
                    <th className="px-10 py-6">Commitment Status</th>
                      ...
                      <tr key={file.fileNo} className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="px-10 py-8">
                          {data && data.isOverdue ? (
                            ...
                            <div>
                              <div className="text-xs font-black text-slate-900">{formatCurrency(nextUpcoming.receivable || 0)}</div>
                              <div className="text-[9px] font-bold uppercase text-slate-400 mt-0.5 flex items-center gap-1">
                                <Clock size={10} /> {nextUpcoming.duedate}
                              </div>
                            </div>
                          ) : (
        ...
        <div className="xl:col-span-4 space-y-6">
          <div className={`rounded-[3.5rem] p-8 sm:p-10 text-white relative overflow-hidden shadow-2xl flex flex-col min-h-[550px] border transition-all duration-500 ${currentAlert?.isOverdue ? 'bg-[#1a0a0d] border-rose-500/20 shadow-rose-900/20' : 'bg-[#0b1424] border-white/5 shadow-slate-900/20'}`}>
                  ...
                  <div className="space-y-4 flex-1">
                    <div className={`bg-white/5 border p-8 rounded-[2.5rem] backdrop-blur-sm transition-all ${currentAlert.isOverdue ? 'border-rose-500/30' : 'border-white/10'}`}>
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">
                        {currentAlert.isOverdue ? 'Total Overdue Balance' : 'Amount Outstanding'}
                      </p>
                      <p className={`text-4xl font-black ${currentAlert.isOverdue ? 'text-rose-500' : 'text-white'}`}>
                        {formatCurrency(currentAlert.isOverdue ? currentAlert.totalOverdueAmount : (currentAlert.nextUpcoming?.receivable || 0))}
                      </p>
                      {currentAlert.isOverdue && (
                        <p className="text-[9px] font-black text-rose-300 uppercase tracking-widest mt-2">
                          Cumulative sum of {currentAlert.overdueTransactions.length} missed payments
                        </p>
                      )}
                    </div>
                    
                    {currentAlert.isOverdue ? (
                      <div className="bg-white/5 border border-rose-500/20 p-6 rounded-[2rem] max-h-[140px] overflow-y-auto custom-scrollbar">
                        <div className="flex items-center gap-2 mb-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                          <History size={12} /> Missed Installments
                        </div>
                        <div className="space-y-2">
                          {currentAlert.overdueTransactions.map((t, i) => (
                            <div key={i} className="flex justify-between items-center text-[11px]">
                              <span className="font-bold text-slate-300 uppercase">{t.duedate}</span>
                              <span className="font-black text-rose-400">{formatCurrency(t.balduedeb || 0)}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className={`flex items-center gap-5 px-4 py-6 bg-white/5 rounded-3xl border transition-all ${currentAlert.isOverdue ? 'border-rose-500/30' : 'border-white/10'}`}>
                        ...
                        <div>
                          ...
                          <p className={`text-lg font-black ${currentAlert.isOverdue ? 'text-rose-200' : 'text-white'}`}>
                            {currentAlert.isOverdue ? currentAlert.overdueTransactions[0].duedate : currentAlert.nextUpcoming?.duedate}
                          </p>
                        </div>
                      </div>
                    )}
